...
export function parseChangeset(text, path = "changeset") {
  ...
  if (!releases || typeof releases !== "object" || Array.isArray(releases))
    throw new Error(`${path}: frontmatter must be a package map`)
  const entries = Object.entries(releases)
  if (entries.length !== 1 || entries[0][0] !== PACKAGE)
    throw new Error(`${path}: only ${PACKAGE} may declare release intent`)
