...
test("aggregates the highest pending Changeset bump", () => {
  assert.equal(planRelease({ baseVersion: "0.1.29", changesets: [changeset("patch"), changeset("minor")] }).version, "0.2.0");
});

test("allows an explicit bump to raise but not lower intent", () => {
  assert.equal(planRelease({ baseVersion: "0.1.29", changesets: [changeset("patch")], requestedBump: "major" }).version, "1.0.0");
  assert.throws(() => planRelease({ baseVersion: "0.1.29", changesets: [changeset("major")], requestedBump: "minor" }), /cannot lower/);
});
