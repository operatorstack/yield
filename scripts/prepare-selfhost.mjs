...
if (!packageName)
  throw new Error(`unsupported self-host platform ${process.platform}/${process.arch}`)
const runtime = await realpath(require.resolve(packageName))
const modules = await realpath(resolve(root, "node_modules"))
if (runtime !== modules && !runtime.startsWith(`${modules}${sep}`))
  throw new Error("refusing to chmod a runtime outside this repository's node_modules")
const details = await stat(runtime)
if (!details.isFile() || details.size === 0)
  throw new Error("published Yield runtime is missing or empty")
if (process.platform !== "win32" && (details.mode & 0o111) === 0)
  await chmod(runtime, details.mode | 0o755)
