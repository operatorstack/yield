...
test("Python README example matches the tested environment doctor", async () => {
  ...
  const readmeProgram = readmeMatch[1].replace(/^from yieldskill import define_skill\n+/, "").trim()
  assert.equal(readmeProgram, sourceMatch[1].trim())
})
...
test("Python README presents a public five-step workflow", async () => {
  ...
  assert.match(
    readme,
    /https:\/\/github\.com\/operatorstack\/yield\/blob\/main\/docs\/reference\/cli\.md/,
  )
...
test("README agent claims match the pinned registry", async () => {
  ...
  assert.deepEqual(verified.map((agent) => agent.id).sort(), ["claude-code", "codex", "cursor"])
  assert.match(normalized, /Verified with Cursor, Codex, and Claude Code\./)
  assert.match(
    normalized,
    new RegExp(
      `Registry-backed project paths are available for ${registryBacked.length} more coding agents\\.`,
    ),
  )
...
test("README uses the borderless Yield mark", async () => {
  const [readme, mark] = await Promise.all([text("README.md"), text("assets/yield-mark.svg")])
  assert.match(
    readme,
...
test("README and quickstart use the public documentation and package registries", async () => {
  const [readme, pythonReadme, rustReadme, goReadme, docsIndex, quickstart, agentSetup] =
    await Promise.all([
      text("README.md"),
      text("sdk/python/README.md"),
      text("sdk/rust/README.md"),
  ...
  assert.match(
    readme,
    /href="https:\/\/pkg\.go\.dev\/github\.com\/operatorstack\/yield\/sdk\/yield">pkg\.go\.dev<\/a>/,
  )
...
test("root README links survive npm package rendering", async () => {
  ...
  assert.match(
    readme,
    /https:\/\/github\.com\/operatorstack\/yield\/blob\/main\/docs\/skill-workflows\.md/,
  )
  assert.match(readme, /https:\/\/github\.com\/operatorstack\/yield\/blob\/main\/evals\/README\.md/)
  assert.match(
    readme,
    /https:\/\/github\.com\/operatorstack\/yield\/tree\/main\/examples\/library\//,
  )
