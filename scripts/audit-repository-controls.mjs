...
export function auditRepositoryControls({ workflow, actions, protection, rulesets }) {
  ...
  return { requiredChecks: requiredChecks.length, immutableTagRuleset: tagRule.id };
}
...
async function main() {
  ...
  const [workflow, actions, protection, rulesets] = await Promise.all([
    github("actions/permissions/workflow"),
    github("actions/permissions"),
    github("branches/main/protection"),
    github("rulesets"),
  ]);
  const result = auditRepositoryControls({ workflow, actions, protection, rulesets });
  console.log(`repository-controls: ${result.requiredChecks} required checks and immutable tag ruleset ${result.immutableTagRuleset} verified`);
}
