...
function cargoVersions(text, expectedName) {
  ...
  for (const line of text.split("\n").filter(Boolean)) {
    ...
    if (record.name !== expectedName)
      throw new Error(`Cargo index for ${expectedName} contains ${record.name}`)
...
export async function verifyRegistryHistory({ versions, base, fetchImpl = fetch }) {
  ...
  const goVersions = new Set(
    (await fetchText(fetchImpl, `${base}/go/github.com/operatorstack/yield/@v/list`))
      .split(/\s+/)
      .filter(Boolean),
  )
  for (const version of versions) {
    if (!goVersions.has(`v${version}`))
      missing.push(`Go github.com/operatorstack/yield@v${version}`)
  }

  const rustNames = ["yieldskill", ...targets.map(rustPackage)]
  for (const name of rustNames) {
    const available = cargoVersions(
      await fetchText(fetchImpl, `${base}/cargo/index/${cargoPath(name)}`),
      name,
    )
    for (const version of versions) {
...
if (process.argv[1] && import.meta.filename === process.argv[1]) {
  verifyRegistryHistory({ ...parseArgs(process.argv.slice(2)) })
    .then((result) =>
      console.log(
        `verified ${result.versions.length} release versions across 4 SDKs and ${result.targets.length} targets`,
      ),
    )
    .catch((error) => {
      console.error(`verify-registry-history: ${error.message}`)
      process.exit(1)
    })
}
