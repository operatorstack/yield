...
test("npm initializer forwards bootstrap and user arguments to the matching CLI", async (t) => {
  ...
  await cp(join(import.meta.dirname, "create-yield/bin/create-yield.mjs"), join(initializer, "bin/create-yield.mjs"));
  ...
  execFileSync(process.execPath, [join(initializer, "bin/create-yield.mjs"), "--root", root, "--dry-run"], {
    cwd: root,
    env: { ...process.env, RECEIPT: receipt },
  });
  assert.deepEqual(JSON.parse(await readFile(receipt, "utf8")), [
    "bootstrap", "--language", "typescript", "--root", root, "--dry-run",
  ]);
});
