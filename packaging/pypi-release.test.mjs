...
test("accepts matching remote files and returns only missing wheels", async (t) => {
  ...
  assert.throws(
    () => compareRelease(local, [{ filename: local[0].filename, sha256: "wrong" }]),
    /hash mismatch/,
  )
  assert.throws(
    () => compareRelease(local, [{ filename: "yieldskill-1.2.3.tar.gz", sha256: "x" }]),
    /unexpected remote file/,
  )
})
...
test("prepares an idempotent upload directory", async (t) => {
  ...
  assert.deepEqual(
    (await readdir(upload)).sort(),
    partial.missing.map(({ filename }) => filename).sort(),
  )
