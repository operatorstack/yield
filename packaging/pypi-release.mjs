...
function parseArgs(argv) {
  ...
  expect(["inspect", "prepare", "verify"].includes(command), "command must be inspect, prepare, or verify");
...
export async function inspectLocalRelease(directory, version) {
  ...
  expect(JSON.stringify(actual) === JSON.stringify(expected), `wheel set mismatch: expected ${expected.join(", ")}; got ${actual.join(", ")}`);
  return Promise.all(actual.map(async (filename) => ({ filename, sha256: await sha256(join(directory, filename)) })));
}
...
export function compareRelease(local, remote) {
  ...
  for (const file of remote) {
    ...
    expect(localByName.get(file.filename) === file.sha256, `remote hash mismatch for ${file.filename}`);
  }
...
export async function fetchPyPIRelease(version, fetchImpl = fetch) {
  ...
  return (payload.urls ?? []).map((file) => ({ filename: file.filename, sha256: file.digests?.sha256 ?? "" }));
}
...
async function main() {
  ...
  if (options.command === "prepare") {
    ...
    await appendOutput(options.output, { publish: result.missing.length > 0, missing_count: result.missing.length });
    process.stdout.write(`${JSON.stringify({ version, missing: result.missing.map(({ filename }) => filename) }, null, 2)}\n`);
    return;
  }
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    if (attempt === attempts) throw new Error(`PyPI release is incomplete: ${missing.map(({ filename }) => filename).join(", ")}`);
    await new Promise((resolveDelay) => setTimeout(resolveDelay, delayMs));
  }
}

if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error) => { console.error(`pypi-release: ${error.message}`); process.exit(1); });
}
