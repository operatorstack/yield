...
function registry({ omit = "" } = {}) {
  return async (url) => {
    if (url.includes("/npm/")) {
      ...
      const present = Object.fromEntries(versions.filter((version) => `${name}@${version}` !== omit).map((version) => [version, {}]));
      return response(JSON.stringify({ name, versions: present }));
    }
    if (url.includes("/pip/simple/yieldskill/")) {
      const files = versions.flatMap((version) => targets.map((target) => `yieldskill-${version}-py3-none-${target.pythonTag}.whl`));
    ...
    if (url.includes("/cargo/index/")) {
      ...
      const records = versions.filter((version) => `Cargo ${name}@${version}` !== omit).map((vers) => JSON.stringify({ name, vers }));
...
test("verifies every SDK, platform package, wheel, and runtime crate", async () => {
  const result = await verifyRegistryHistory({ versions, base: "https://registry.test", fetchImpl: registry() });
...
test("reports a missing historical version instead of accepting latest", async () => {
  ...
  await assert.rejects(
    verifyRegistryHistory({ versions, base: "https://registry.test", fetchImpl: registry({ omit: missing }) }),
    new RegExp(missing),
  );
});
