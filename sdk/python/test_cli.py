...
class CliTest(unittest.TestCase):
    ...
    def test_unix_replaces_process_and_forwards_arguments(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(call.args[2]["PATH"].split(os.pathsep)[0], str(Path(os.sys.executable).absolute().parent))
    ...
    def test_windows_preserves_exit_code_and_arguments(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(call.kwargs["env"]["PATH"].split(os.pathsep)[0], str(Path(os.sys.executable).absolute().parent))
