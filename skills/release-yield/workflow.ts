...
function command(ctx: ReleaseContext, id: string, args: string, claim: string, timeout = 600): Receipt {
...
function stringField(ctx: ReleaseContext, receipt: Receipt, field: string): string {
  ...
  ctx.require(typeof value === "string" && value.length > 0, `controller receipt contains ${field}`, receipt);
  return value as string;
}

function matchingField(ctx: ReleaseContext, receipt: Receipt, field: string, pattern: RegExp): string {
...
export function runReleaseYield(ctx: ReleaseContext) {
  ...
  const dry = command(ctx, "dispatch-dry-run", `dispatch --bump ${bump} --dry-run true`, "the dry-run workflow is dispatched");
  const dryRunID = matchingField(ctx, dry, "run_id", /^\d+$/);
  const dryResult = command(ctx, "wait-dry-run", `wait --run-id ${dryRunID}`, "the GitHub dry run succeeds", 1800);
  ctx.require(stringField(ctx, dryResult, "source_sha") === sourceSha, "the dry run uses the preflight source SHA", dryResult);
  ...
  ctx.require(stringField(ctx, live, "source_sha") === sourceSha, "the live dispatch uses the authorized source SHA", live);
  const releaseRunID = matchingField(ctx, live, "run_id", /^\d+$/);
  const publisherBaseline = matchingField(ctx, live, "publisher_baseline", /^(?:none|\d+(?:,\d+)*)$/);
  const finalizerBaseline = matchingField(ctx, live, "finalizer_baseline", /^(?:none|\d+(?:,\d+)*)$/);
