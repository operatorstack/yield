...
function parseArgs(argv) {
  ...
  for (let index = 0; index < rest.length; index += 2) {
    if (!rest[index]?.startsWith("--") || rest[index + 1] === undefined)
      throw new Failed(`invalid argument ${rest[index] ?? ""}`)
...
export function selectNewRun(runs, baseline, { event, sourceSha } = {}) {
  ...
  const candidates = runs
    .filter((run) => !previous.has(String(run.databaseId)))
    .filter((run) => !event || run.event === event)
    .filter((run) => !sourceSha || run.headSha === sourceSha)
  if (candidates.length > 1)
    throw new Blocked("multiple matching workflow runs appeared; refusing ambiguous correlation")
...
function listRuns(workflow) {
  return ghJSON(
    ...
    "50",
    "--json",
    "databaseId,status,conclusion,headSha,event,createdAt,url,displayTitle",
  )
}
  ...
  try {
    ghInput(
      ...
      JSON.stringify({
        environment_ids: ids,
        state: "approved",
        comment: "Authorized by the recorded release-yield decision.",
      }),
    )
  } catch {
    throw new Blocked(
...
async function waitRun(runID, expectedEnvironments = new Set()) {
  ...
  for (let attempt = 0; attempt < 1800; attempt += 1) {
    for (const name of approve(runID, pendingDeployments(runID), expectedEnvironments))
      seen.add(name)
    ...
    if (runInfo.status === "completed") {
      if (runInfo.conclusion !== "success")
        throw new Failed(`workflow run ${runID} concluded ${runInfo.conclusion}`)
...
async function preflight(values) {
  ...
  if (normalizeRemote(git("remote", "get-url", "origin")) !== `https://github.com/${repository}`)
    throw new Blocked(`origin is not ${repository}`)
  if (git("branch", "--show-current") !== "main")
    throw new Blocked("release-yield must run from the main branch")
  if (git("status", "--porcelain")) throw new Blocked("the worktree is not clean")
  git("fetch", "origin", "main")
  const sourceSha = git("rev-parse", "HEAD")
  if (git("rev-parse", "origin/main") !== sourceSha)
    throw new Blocked("main does not match origin/main")
  let protection
  try {
    gh("auth", "status")
  ...
  if (!protection?.enforce_admins?.enabled || !protection?.required_status_checks?.strict)
    throw new Blocked("main is not protected with strict required checks")
  const conflicting = [...listRuns("release.yml"), ...listRuns("npm-publish.yml")].filter((item) =>
    active.has(item.status),
  )
  if (conflicting.length)
    throw new Blocked(`another release workflow is active: ${conflicting[0].url}`)
...
async function dispatch(values) {
  ...
  if (git("branch", "--show-current") !== "main" || git("status", "--porcelain"))
    throw new Blocked("main changed after preflight")
  if (git("rev-parse", "origin/main") !== sourceSha)
    throw new Blocked("source SHA changed after preflight")
  const conflicting = [...listRuns("release.yml"), ...listRuns("npm-publish.yml")].filter((item) =>
    active.has(item.status),
  )
  if (conflicting.length)
  ...
  const found = await discover("release.yml", releaseBaseline, {
...
async function monitorController(values) {
  ...
  const publisher = await discover(
    "npm-publish.yml",
    values["publisher-baseline"] === "none" ? "" : values["publisher-baseline"],
    { event: "workflow_dispatch" },
  )
  return {
    run_id: String(info.databaseId),
    run_url: info.url,
    publisher_run_id: String(publisher.databaseId),
    publisher_run_url: publisher.url,
...
async function monitorFinalizer(values) {
  const found = await discover(
    ...
    values.baseline === "none" ? "" : values.baseline,
    { event: "workflow_run" },
  )
...
async function verify(values) {
  ...
  if (
    ...
    throw new Failed("invalid release identity")
  ...
  if (
    python?.info?.version !== version ||
    !Array.isArray(python.urls) ||
    python.urls.length !== 6 ||
    python.urls.some((file) => !file?.digests?.sha256)
  )
    throw new Failed(`yieldskill ${version} is incomplete on PyPI`)
  for (const name of crates) {
    const response = await fetch(`https://crates.io/api/v1/crates/${name}/${version}`)
    const body = await response.json()
    if (!response.ok || body?.version?.num !== version)
      throw new Failed(`${name}@${version} is missing from crates.io`)
  }
  run("node", [
    "packaging/go-release.mjs",
    "--version",
    version,
    "--source-sha",
    sourceSha,
  ...
  if (remoteTag !== sourceSha)
    throw new Failed(`${tag} does not point to the authorized source SHA`)
  const release = ghJSON(
    ...
    repository,
    "--json",
    "isDraft,tagName,url,targetCommitish",
  )
  if (release.isDraft || release.tagName !== tag)
    throw new Failed(`${tag} is not a finalized GitHub release`)
  return {
    targets: {
...
async function main() {
  try {
    ...
    process.stdout.write(
      `${JSON.stringify({ status: "ok", ...(await execute(action, values)) })}\n`,
    )
  } catch (error) {
