...
test("correlates one new workflow dispatch at the exact source SHA", () => {
  assert.equal(selectNewRun(runs, "1,2", { event: "workflow_dispatch", sourceSha: "abc" }).databaseId, 3);
});
...
test("refuses ambiguous new runs", () => {
  assert.throws(
    () => selectNewRun([...runs, { databaseId: 4, event: "workflow_dispatch", headSha: "abc" }], "1,2", { event: "workflow_dispatch", sourceSha: "abc" }),
    Blocked,
  );
});
