...
async function runCodex({ repo, authHome, prompt, evidenceDir, outputSchema, outputFile }) {
  const args = [
    ...
    "--json", "--sandbox", "danger-full-access", "-C", repo,
...
async function prepareRepository(session, yskill) {
  ...
  await cp(join(conversionRoot, "fixtures/source-skill/SKILL.md"), join(repo, "skills/source-release/SKILL.md"))
  ...
  command(yskill, ["bootstrap", "--root", repo, "--language", "go", "--agent", "codex", "--yes"], repo)
...
async function builderEvidence(repo) {
  ...
  for (const name of logs.sort()) {
    ...
    const semantic = events.some((event) => event.type === "operation.completed" && event.data?.request_id === "project-semantics")
  ...
  const terminal = events.findLast((event) => event.type === "run.completed" || event.type === "run.blocked" || event.type === "run.refused")
  if (terminal?.type !== "run.completed") throw new Error(`builder terminal was ${terminal?.type ?? "missing"}`)
  const projected = events.find((event) => event.type === "operation.completed" && event.data?.request_id === "project-semantics")
...
function addUsage(left, right) {
  return Object.fromEntries(["input_tokens", "cached_input_tokens", "output_tokens", "reasoning_output_tokens"].map((key) => [key, (left[key] ?? 0) + (right[key] ?? 0)]))
}
...
async function main() {
  ...
  try {
    ...
    await cp(join(conversionRoot, "fixtures/negative-control"), join(repo, ".eval/negative-control"), { recursive: true })
    await cp(join(conversionRoot, "fixtures/fault-probes.json"), join(repo, ".eval/fault-probes.json"))
    await writeFile(join(repo, ".eval/projection.json"), JSON.stringify(evidence.projection, null, 2) + "\n")
    ...
    const counts = Object.fromEntries(["control", "guidance", "both", "excluded"].map((kind) => [kind, clauses.filter((clause) => clause.disposition === kind).length]))
    const findings = judge.clause_findings ?? []
    const passed = judge.candidate?.verdict === "accept" && judge.negative_control?.verdict === "reject" &&
      clauses.length === 4 && Object.values(counts).every((count) => count === 1) &&
      findings.length === 4 && findings.every((finding) => finding.preserved === true && finding.reachable === true) &&
    ...
    const receipt = {
      ...
      model: { product: "Codex CLI", cli_version: command("codex", ["--version"]), name: model, reasoning },
      sessions: 2,
      ...
      claim_boundary: "Advisory evidence for this four-clause fixture. The contract is stable; model projections can differ.",
    }
    if (process.argv.includes("--write")) await writeFile(resultPath, JSON.stringify(receipt, null, 2) + "\n")
    console.log(JSON.stringify({ status: receipt.status, clauses: receipt.clause_counts, evidence: relative(evalRoot, evidenceRoot) }, null, 2))
