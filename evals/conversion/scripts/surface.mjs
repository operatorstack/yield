...
export function isSemanticPath(path) {
  ...
  return normalized === receiptSurface || semanticSurface.some((entry) => entry.endsWith("/") ? normalized.startsWith(entry) : normalized === entry)
}
...
export function changedPaths(base, head) {
  ...
  return execFileSync("git", ["diff", "--name-only", effectiveBase, head], { cwd: yieldRoot, encoding: "utf8" })
