...
defineSkill((ctx) => {
  ...
  if (!safe(spec.destination, true) || spec.destination === "skills/yield-workflow-builder" || existsSync(resolve(root,spec.destination))) ctx.blocked("the destination must be a new path under skills/");
  let source = "";
  if (mode === "convert") {
  ...
  const flow = ctx.agentTask("extract-flow", "Extract or design the minimal workflow control flow. Keep model judgment in agent_task operations. Put order, branches, commands, approvals, evidence requirements, and finish rules in code.", {mode,spec,source}, flowSchema);
  let written = ctx.agentTask<Files>("write-workflow", "Create the complete Yield skill workflow at the destination. Write the language program, thin SKILL.md, exact-version dependencies, skill.json when required, and self-contained fixtures. Do not edit files outside the destination. Return every file written.", {mode,spec,flow}, filesSchema);
  ...
  for (let attempt=1; checked.exit_code!==0 && attempt<=2; attempt++) {
    written = ctx.agentTask<Files>("repair-generated-"+attempt, "The generated workflow failed verification. Fix only the destination files and return every changed file.", {spec,stdout:checked.stdout,stderr:checked.stderr}, filesSchema);
...
def program(ctx):
    ...
    if not safe(spec["destination"], True) or spec["destination"] == "skills/yield-workflow-builder" or destination.exists():
    ...
    flow = ctx.agent_task("extract-flow", "Extract or design the minimal workflow control flow. Keep model judgment in agent_task operations. Put order, branches, commands, approvals, evidence requirements, and finish rules in code.", context={"mode":mode,"spec":spec,"source":source}, schema=FLOW_SCHEMA)
    written = ctx.agent_task("write-workflow", "Create the complete Yield skill workflow at the destination. Write the language program, thin SKILL.md, exact-version dependencies, skill.json when required, and self-contained fixtures. Do not edit files outside the destination. Return every file written.", context={"mode":mode,"spec":spec,"flow":flow}, schema=FILES_SCHEMA)
    ...
    for attempt in range(1, 3):
        ...
        written = ctx.agent_task(f"repair-generated-{attempt}", "The generated workflow failed verification. Fix only the destination files and return every changed file.", context={"spec":spec,"stdout":checked.stdout,"stderr":checked.stderr}, schema=FILES_SCHEMA)
...
func main() { yield.Main(func(ctx *yield.Context) (yield.Outcome,error) {
  ...
  root,err:=filepath.Abs(filepath.Join("..","..")); if err!=nil{return yield.Outcome{},err};root,err=filepath.EvalSymlinks(root);if err!=nil{return yield.Outcome{},err}; destination:=filepath.Join(root,spec.Destination); if !safe(root,spec.Destination,true)||spec.Destination=="skills/yield-workflow-builder" { return yield.Outcome{},ctx.Blocked("the destination must be a new path under skills/") }; if _,err=os.Stat(destination);err==nil{return yield.Outcome{},ctx.Blocked("the destination must be a new path under skills/")}else if !os.IsNotExist(err){return yield.Outcome{},err}
  source:=""; if mode=="convert" { if spec.SourcePath==""||!safe(root,spec.SourcePath,false){return yield.Outcome{},ctx.Blocked("the source skill must be inside the repository")}; b,readErr:=os.ReadFile(filepath.Join(root,spec.SourcePath,"SKILL.md"));if readErr!=nil{return yield.Outcome{},ctx.Blocked("the source SKILL.md does not exist")};source=string(b) }
  flow:=ctx.AgentTask("extract-flow","Extract or design the minimal workflow control flow. Keep model judgment in agent_task operations. Put order, branches, commands, approvals, evidence requirements, and finish rules in code.",map[string]any{"mode":mode,"spec":spec,"source":source},json.RawMessage(flowSchema))
  writtenRaw:=ctx.AgentTask("write-workflow","Create the complete Yield skill workflow at the destination. Write the language program, thin SKILL.md, exact-version dependencies, skill.json when required, and self-contained fixtures. Do not edit files outside the destination. Return every file written.",map[string]any{"mode":mode,"spec":spec,"flow":json.RawMessage(flow)},json.RawMessage(filesSchema));var written fileResult;_ = json.Unmarshal(writtenRaw,&written)
...
fn program(ctx:&mut Context)->SkillResult{
 ...
 let destination=spec["destination"].as_str().unwrap_or("");let root=PathBuf::from("../..").canonicalize().expect("repository root must be readable");if !safe(&root,destination,true)||destination=="skills/yield-workflow-builder"||root.join(destination).exists(){return Err(ctx.blocked("the destination must be a new path under skills/"))}
 let mut source=String::new();if mode=="convert"{let source_path=spec["source_path"].as_str().unwrap_or("");if !safe(&root,source_path,false){return Err(ctx.blocked("the source skill must be inside the repository"))};source=fs::read_to_string(root.join(source_path).join("SKILL.md")).map_err(|_|ctx.blocked("the source SKILL.md does not exist"))?;}
 let flow=ctx.agent_task("extract-flow","Extract or design the minimal workflow control flow. Keep model judgment in agent_task operations. Put order, branches, commands, approvals, evidence requirements, and finish rules in code.",Some(json!({"mode":mode,"spec":spec,"source":source})),Some(serde_json::from_str(FLOW_SCHEMA).unwrap()));
